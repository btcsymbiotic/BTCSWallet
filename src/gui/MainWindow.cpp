...
void MainWindow::setMainWindowTitle() {
  setWindowTitle(QString(tr("BTCS Wallet %1")).arg(Settings::instance().getVersion()));
}
...
void MainWindow::createLanguageMenu(void)
{
  ...
  m_langPath = "/usr/local/share/BTCS/BTCSWallet/languages";
#else
  m_langPath = "/opt/BTCS/languages";
#endif
  QDir dir(m_langPath);
...
//  QMessageBox::information(nullptr, QObject::tr("Help"), cmdLineParser.getHelpText());
    QMessageBox *msg = new QMessageBox(QMessageBox::Information, QObject::tr("Help"),
                       ...
                       QMessageBox::Ok, this);
    msg->setInformativeText(tr("More info can be found at BTCS.Net in Help section"));
...
void MainWindow::walletOpened(bool _error, const QString& _error_text) {
  if (!_error) {
	...
	setWindowTitle(QString(tr("%1 - BTCS Wallet %2")).arg(Settings::instance().getWalletFile()).arg(Settings::instance().getVersion()));
...
void MainWindow::walletClosed() {
  ...
  setWindowTitle(QString(tr("BTCS Wallet %2")).arg(Settings::instance().getVersion()));
...
void MainWindow::createTrayIcon()
{
    ...
    QString toolTip = QString(tr("BTCS Wallet %1")).arg(Settings::instance().getVersion());
