...
double SendFrame::getMinimalFee() {
  ...
  double scale = pow(10., floor(log10(fabs(fee))) + (3 - digits));
...
void SendFrame::parsePaymentRequest(QString _request) {
    ...
    if(_request.startsWith("BTCS://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "BTCS:");
    }
    if(!_request.startsWith("BTCS:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with BTCS:"), QtCriticalMsg));
      return;
    }
  ...
  ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "PARSGnjdcRG4gY9g4rMTFAEHZLGU7uK8YMiFY3Do1uzoMz4LMA6PqmdPp7ZxDu25b56RyhCevkWjbAMng532iFFj8L5RaPyT4s";
